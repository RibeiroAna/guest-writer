...
**Important**: the audience should be `http://localhost:3000` (if you choose another one, you will have to change some pieces of code to your chosen address later on) and the signing algorithm **must** be RS256 (if it is another one, the verification method can be quite different).
...
```
If everything was done right, you will get as the answer of this request a JSON object containing the token, the expiration (86400 seconds or 24 hours) and the token type (bearer). Keep the token for the further calls to our API.
